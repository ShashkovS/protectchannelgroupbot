...
async def on_shutdown(app):
    ...
    if config.production_mode:
        ...
        if all_async_tasks_but_current:
            await asyncio.wait(all_async_tasks_but_current, timeout=10)
    db.sql.disconnect()
...
def prepare_app():
    ...
    print('config.production_mode', config.production_mode)
    if config.production_mode:
        config.url_prefix = f'https://{config.webhook_host}'
    else:
        config.url_prefix = f'http://127.0.0.1:{LOCAL_APP_PORT}'
    logger.info('Routes:')
    for route in app.router.routes():
        logger.info(f'{route.method}: {config.url_prefix}{route.resource.canonical}')
        print(f'{route.method}: {config.url_prefix}{route.resource.canonical}')
